...
Complexity: matmuls dominate at $\Theta(\chi^3 D^4)$ vs exact $\Theta(\chi^3 D^6)$ — the $\Theta(D^2)$ speedup. **v1 ships the explicit-matrix form** (the fat tensor is materialized by absorption anyway); **v1.1** adds the factored `LinearOp` where `Wop @ v` is evaluated as two einsums against `(carry, M, a)` at $\Theta(\chi^2 D^4)$/vector without materializing `Wmat`, cutting peak memory from $\Theta(\chi^2 D^4)$ to $\Theta(\chi D^4)$ — activate when the $D$-ladder hits memory limits, guarded by an equivalence test against v1.
...
| Zip-up, sketched | $\Theta(L^2\,\chi^3 D^4) \to \Theta(L^2 D^{10})$ | $\Theta(\chi^2 D^4)$ v1; $\Theta(\chi D^4)$ v1.1 factored |
